...
**Architecture:** A single hidden-code marimo cell owns a function-local `collect_provenance()` helper and renders its ordered result as Markdown. The PDF workflow uses two executions: an intentional environment-migration refresh followed by a clean deterministic export whose provenance and generated outputs are verified before the PDF enters the repository.
...
- Produces: `marimo>=0.23.16` in project dependencies and a `dev` dependency group containing `playwright>=1.62.0`; an environment capable of invoking WebPDF.
...
- [ ] **Step 4: Inspect and commit only the dependency update**
...
Expected: the commit contains the user's marimo lock refresh, the raised marimo floor, and the new Playwright development group; no generated analysis output is staged.
...
**Interfaces:**
- Consumes: `load_notebook_function(name)` from the existing AST-based test harness.
- Produces: `collect_provenance(notebook_path="diabetes/analysis_v2.py", *, now=None, package_version=None, run_git=None) -> dict[str, str]` and a visible Markdown provenance table.
...
def _(mo):
    def collect_provenance(
        notebook_path="diabetes/analysis_v2.py",
        *,
        now=None,
    ...
    _provenance_rows = "\n".join(
        f"| {label} | `{value}` |" for label, value in _provenance.items()
    )
    mo.md(
        f"""
...
Keep the raw simulation data in `diabetes/results/`. This directory is intentionally git-ignored and is not included in the repository or the PDF snapshot.
...
Generate a candidate PDF outside the repository so failed or unreviewed runs cannot replace the published snapshot:

```bash
uv run marimo export pdf --include-outputs --webpdf --no-include-inputs --no-rasterize-outputs diabetes/analysis_v2.py -o /tmp/analysis_v2_last_run.pdf -f
```
...
```bash
rg -n "analysis\.py|analysis_v2|analysis_v2_last_run|playwright|include-outputs|webpdf" README.md
git diff --check
```

Expected: no `diabetes/analysis.py` reference remains; the notebook path, PDF path, prerequisites, and exact export flags are present.
...
git ls-files -z diabetes/figures diabetes/csv_results | xargs -0 sha256sum > /tmp/analysis_v2_pre_refresh.sha256
```

Expected: the tracked worktree is clean before execution; 36 output hashes are recorded.
...
```bash
uv run marimo export pdf --include-outputs --webpdf --no-include-inputs --no-rasterize-outputs diabetes/analysis_v2.py -o /tmp/analysis_v2_baseline_refresh.pdf -f
```

Expected: exit code 0 and a nonempty temporary PDF. Differences in tracked PNG and CSV files are expected on this first upgraded run.
...
Expected: CSV comparison reports no semantic failures. PNG differences are quantified separately from metadata changes.
...
Expected: clean tracked worktree and 36 hashes.
...
Expected: exit code 0 and a nonempty PDF.
...
Expected: every hash reports `OK`; Git status is empty. Stop if either condition fails.
...
rg -n "Execution provenance|Executed at|Python|marimo|NumPy|pandas|Matplotlib|SciPy|Git commit|Git state|Notebook SHA-256|clean" /tmp/analysis_v2_last_run.txt
pdfimages -list /tmp/analysis_v2_last_run.pdf
stat -c "%n %s bytes" /tmp/analysis_v2_last_run.pdf
```

Expected: all provenance fields are searchable, `Git state` is `clean`, PDF images are present, and the file is below GitHub's 100 MB file limit.
...
Inspect every rendered page with the image viewer. Expected: the provenance table is near the beginning; figures, captions, statistical output, and page breaks are legible; no page is empty or clipped.
...
Expected: `diabetes/results/` is ignored, no result file is tracked, and `run_model.py` has no diff.
...
pdftotext diabetes/analysis_v2_last_run.pdf - | rg "Execution provenance|Git state|clean|Notebook SHA-256"
git status --short --branch
git log -6 --oneline
```

Expected: the PDF remains readable and searchable, the worktree is clean, and all task-scoped commits are present locally. Do not push.
